...
`media` follows the same hash-gated rewrite as `data`: it is only recomputed and rewritten when the row's `hash` actually changes, and it is never left NULL for a row this loader writes. One backfill exception applies to the batch path: a row persisted before this column existed has a `hash` its payload already agrees with, so the hash-gated rewrite would never fire for it again. `batch_processor.py` therefore reads `media IS NULL` alongside the hash and, for an unchanged-hash row that still lacks the column, issues an `UPDATE ... SET media, updated_at` — `hash` and `data` are untouched. A `force_reprocess` run on an upgraded stack fills the column across the existing catalog that way; those rows are reported as `media_backfilled`, separately from `skipped`. `data` itself is never rewritten to hold or omit `media` beyond whatever the source event carried — this column is additive, computed separately.
