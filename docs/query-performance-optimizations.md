...
1. **Phase 2 (Opt-9):** Pre-computed `first_year` property on Genre/Style nodes during import, with RANGE indexes:
   ```cypher
...
1. NodeIndexSeekByRange on r.year > 0 → **16,427,716 rows**
1. **CartesianProduct** → 16M rows
1. Expand(Into) → **134M DB hits**
...
1. **Phase 2 (Opt-7):** Pattern comprehension `[(g)<-[:IS]-(r:Release) WHERE r.year > 0 | r.year]` to force genre-first traversal.
1. **Phase 3 (Opt-9):** Redis caching with 24h TTL — genre/style trends data is static between imports.
...
1. Expanded ALL releases in those genres (for "Electronic": 5.6M releases) to find candidate artists
1. For each of 200 candidates, ran 4 separate profile queries = **800 sequential Neo4j queries**
1. Computed cosine similarity
...
1. **Phase 2 (Opt-4):** Two-phase candidate discovery — lightweight ID finding, then batch profile fetch.
1. **Phase 3 (Opt-5):** Top-5-genres LIMIT + per-genre LIMIT 500 to cap mega-genre explosion.
1. **Phase 4 (Opt-6):** CALL {} subqueries per-genre to prevent cross-genre row multiplication.
1. **Phase 5 (Opt-8):** Style-based similarity (757 styles vs 16 genres = finer granularity, fewer releases per traversal). Split into 25-label batches with concurrent queries.
1. **Phase 6 (Opt-9):** Redis caching with 24h TTL.
...
1. 5 concurrent queries via `asyncio.gather()`: paginated results, total count, type counts, genre facets, decade facets
1. Redis caching with 300s TTL
1. `_TOTAL_COUNT_CAP = 10000` limits auxiliary count queries
